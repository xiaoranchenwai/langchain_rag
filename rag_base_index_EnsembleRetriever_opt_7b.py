...
def setup_enhanced_rag_agent(
    ...
    docstore_path = "/home/user/cx/agents/rag/enhanced_docstore_all_7b.pkl"
    hypothetical_questions_path = "/home/user/cx/agents/rag/hypothetical_questions_all_7b.pkl"

    # 创建或加载 Chroma 向量存储
    vector_store = Chroma(
        collection_name="enhanced_rag_collection_all_7b",
    ...
    def enhanced_retrieve(query: str) -> List[Document]:
        ...
        if enable_context_expansion and context_expander:
            all_docs = context_expander.expand_context(all_docs[:6])
        
        # 按相关性排序并返回top-k
        return all_docs[:6]# 返回最相关的10个文档
...
def run_enhanced_rag_query(query: str, retriever_func) -> Dict[str, Any]:
    """运行增强的RAG查询"""
    try:
        # 先执行检索
...
def process_excel_with_enhanced_rag(
    ...
    for idx, row in df.iterrows():
        ...
        try:
            rag_result = run_enhanced_rag_query(question, retriever_func)
...
if __name__ == "__main__":
    ...
    OUTPUT_EXCEL = "/home/user/cx/agents/rag/enhanced_rag_results_2_7b.xlsx"
    ...
    retriever_func = setup_enhanced_rag_agent(
